...
describe('Start-UI Task Status Updates', () => {
	...
	test('should update task status through UI interaction', async () => {
		...
		// Verify initial state - StatusBar shows: "Done: 0 | In Progress: 0 | Pending: 2"
		let frame = lastFrame();
		expect(frame).toMatch(/Done:\s+0/);
		expect(frame).toMatch(/In Progress:\s+0/);
		expect(frame).toMatch(/Pending:\s+2/);
		...
		frame = lastFrame();
		expect(frame).toMatch(/Done:\s+0/);
		expect(frame).toMatch(/In Progress:\s+1/);
		expect(frame).toMatch(/Pending:\s+1/);
	...
	test('should update multiple tasks through UI', async () => {
		...
		// Verify both updates - StatusBar format
		const frame = lastFrame();
		expect(frame).toMatch(/Done:\s+1/);
		expect(frame).toMatch(/In Progress:\s+1/);
		expect(frame).toMatch(/Pending:\s+0/);
	});
	...
	test('should show updated task in filtered views', async () => {
		...
		// Should see the task in completed view
		let frame = lastFrame();
		expect(frame).toContain('Update me');
		expect(frame).toContain('Completed (1)'); // Navigation shows view count
		...
		frame = lastFrame();
		expect(frame).toContain('Another task');
		expect(frame).not.toContain('Update me');
		expect(frame).toContain('Pending (1)'); // Navigation shows view count
	});
	...
	test('should handle escape from task detail view', async () => {
		...
		// Should return to list view
		let frame = lastFrame();
		expect(frame).not.toContain('Suggested Actions');
		expect(frame).toContain('All Tasks (2)'); // Navigation shows count
	});
});
