...
describe('Start-UI User Interactions', () => {
	...
	test('should navigate through tasks using keyboard', async () => {
		...
		// The StatusBar shows task position as "Task 01/04"
		expect(initialFrame).toMatch(/Task\s+0?1\/\s*0?4/);
		...
		expect(frame).toMatch(/Task\s+0?2\/\s*0?4/); // StatusBar shows task 2 selected
		...
		expect(frame).toMatch(/Task\s+0?3\/\s*0?4/); // StatusBar shows task 3 selected
		...
		frame = lastFrame();
		expect(frame).toContain('Fix bug B');
		expect(frame).toMatch(/Task\s+0?2\/\s*0?4/); // Back to task 2
	});
	...
	test('should update task status using keyboard shortcuts', async () => {
		...
		// UI should reflect the change in StatusBar
		frame = lastFrame();
		expect(frame).toMatch(/In Progress:\s+2/); // Was 1, now 2
	});
	...
	test('should handle page navigation for long task lists', async () => {
		...
		// Should start with Task 1 in StatusBar
		let frame = lastFrame();
		expect(frame).toMatch(/Task\s+0?1\/\s*25/);
		...
		frame = lastFrame();
		expect(frame).toMatch(/Task\s+11\/\s*25/);
